...
import ButtonEdit from '../../components/button_float'
...
export default function SimpleTable(props) {
    ...
    const checkGetData = useSelector(state => state.check_get_product)
        ...
        return () => {
            if (window.confirm(`Bạn có chắc chắn muốn xóa ${name} không ?`)) {
                axios({
                    ...
                    dispatch({ type: "DELETE_PRODUCT", data: res.data._id })
                })
            }
        }
    }
                        ...
                        <TableBody>
                            {productsToShow.map((product, key) => (
                                <TableRow key={key}>
                                    ...
                                    <TableCell align="left"><Link to={'/dashboard/product/' + product._id}><ButtonEdit icon={<AiFillEdit />} color='secondary' size="small" /></Link></TableCell>
                                    <TableCell align="left"><ButtonDelete color="secondary" size="small" icon={<AiFillDelete/>} onClick={deleteProduct(product._id, product.name)} /></TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
